...
test('TC-REGRESSION-04: Регистрация пользователя', async ({ page }) => {
    ...
    // 4. Кликаем на кнопку регистрации
    await page.locator('#register-button').click();
    // 5. Проверяем успешную регистрацию
    ...
    await expect(page.locator('.account', { hasText: user.email })).toHaveText(user.email);
...
test('TC-REGRESSION-05: Невалидный email', async ({ page }) => {
    ...
    await expect(page.locator('.field-validation-error span')).toHaveText(/wrong email/i);
...
test('TC-REGRESSION-06: пустой Email', async ({ page }) => {
    ...
    await expect(page.locator('.field-validation-error span')).toHaveText(/email is required./i);
